 ...
 */
@Service
public class ReopenCountExpression extends AlarmExpressionHandler {

    private final BugRepository bugRepository;

    public ReopenCountExpression(BugRepository bugRepository) {
    ...
    public boolean checkExpression(AlarmResult alarmResult, AlarmBotDTO.Condition condition) {
        ...
        List<Bug> bugs = bugRepository.selectReopenAlarm(startDate, Double.parseDouble(condition.getCondition()), condition.getConType());
        ...
        if (isAlarm) {
            ...
            alarmResult.getAlarmData().add("监测reopen率" + c + condition.getCondition());
